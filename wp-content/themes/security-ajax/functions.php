<?php
/**
 * security-ajax functions and definitions
 *
 * @link https://developer.wordpress.org/themes/basics/theme-functions/
 *
 * @package security-ajax
 */
	if ( ! defined( 'ABSPATH' ) ) {
		exit;
	}
 ...
 */
function security_ajax_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on security-ajax, use a find and replace
		* to change 'security-ajax' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'security-ajax', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'security-ajax' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'security_ajax_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'security_ajax_setup' );
 ...
 */
function security_ajax_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'security_ajax_content_width', 640 );
}
add_action( 'after_setup_theme', 'security_ajax_content_width', 0 );

/**
 * Enqueue scripts and styles.
 */
function security_ajax_scripts() {
	wp_enqueue_style( 'security-ajax-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_enqueue_style( 'security-ajax-style-main', get_template_directory_uri() . '/assets/css/style.min.css', array(), _S_VERSION );


	wp_enqueue_script( 'security-ajax-navigation', get_template_directory_uri() . '/assets/js/main.min.js', array('jquery'), _S_VERSION, true );


}
add_action( 'wp_enqueue_scripts', 'security_ajax_scripts' );
